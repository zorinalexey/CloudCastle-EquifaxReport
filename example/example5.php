...
for ($i = 0; $i < $countReports; $i++) {
     ...
     */
    $report->base_part->contract->deal->sign_deal_cash_source = 0;
    /*
     ...
     */
    $report->base_part->contract->deal->sign_deal_cash_subject = 0;
    /*
     ...
     * 99 - Иной заем (кредит)
     */
    $report->base_part->contract->deal->type = 2;
    ...
    // Если обращение отсутствует, то необходимо удалить данный объект unset($report->information_part);
    /*
     * UID предыдущего события (УИд обращения когда клиент обратился с предложением совершить сделку)
     */
    $report->information_part->application->uid = $uid;
    /*
     ...
     * 99 - Иной заем (кредит)
     */
    $report->information_part->credit->type = 2;
    /*
     * УИд сделки
     */
